...
# stats 用 ECR は常に作成する(空リポジトリは無害・低コスト)。CI(T5)が ECR 先行 apply の
# -target と ecr_stats_repository_url 出力を無条件参照するため、ここを条件化すると CI が壊れる。
# stats の実体(Lambda/Step Functions/Scheduler)は enable_analytics でゲート化する。
module "ecr_stats" {
  source          = "../../modules/ecr"
