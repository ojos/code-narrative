...
- ループ対象の作業では、`acceptance` に**非対話で実行でき、合否が終了コードで判定できる**検証を最低 1 つ含める（[loop-workflow](../loop-workflow.md) の「前提: 完了条件は機械が検証できる形にする」）。満たせない場合は Step 6 で「ループコーディング非対象」と明記した上で通過する。
- 既定言語は日本語
- ユーザー承認を得るまで起票しない
